...
export const walletApi = {
  ...
  bulkAllocate: (allocationData) =>
    api.post("/wallet/bulk-allocate", allocationData),
  deductFunds: (deductionData) =>
    api.post("/wallet/admin/deduct", deductionData),
};
    ...
    return api
      ...
      .catch((error) => {
        ...
        if (error.response) {
          ...
          throw new Error(
            "Network error - server did not respond. Please check if the backend server is running."
          );
        } else {
